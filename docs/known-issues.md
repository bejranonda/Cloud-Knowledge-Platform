...
| 24 | LiveSync materialisation (`sync_monitor._materialise`) only guarded against `..` and could write a CouchDB doc into `.git/` or `.ckp/` — a sync client could corrupt the Git repo (`.git/config`) or the search index | Resolved | New `_safe_target()` resolves the doc path with `Path.is_relative_to` containment and hard-rejects `.git` / `.ckp`, matching `util.safe_path`; regression test drives `_materialise` with protected and escaping paths | v0.5.2 |
