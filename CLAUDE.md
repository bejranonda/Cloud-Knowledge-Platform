...
- Vault containment must use `Path.is_relative_to`, never a string-prefix compare. Sibling vaults can share a slug prefix (`proj-a` / `proj-a-secret`), so a prefix check lets a project token escape via `../`. This was the v0.5.1 traversal fix (`util.safe_path`); don't reintroduce the prefix pattern. Three write paths must all stay containment-checked: `util.safe_path` (API), `webdav._safe_path` (WebDAV), and `sync_monitor._safe_target` (LiveSync→disk, v0.5.2); all reject `.git` (and the sync path also rejects `.ckp`).
